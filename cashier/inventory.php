...
<head>
    ...
    <style>
        ...
        .login-card {
            ...
            align-items: center;
            position: relative;
            transform: translateZ(20px); /* Lifts content slightly */
        }
        ...
        .hologram-form-input {
            width: 100%;
            padding: 22px 20px 10px 20px; /* Adjusted padding for floating label */
            padding-right: 56px;
...
<body>
    <div class="theme-switcher-badge" id="theme-switch-btn">
        <i class="ph ph-moon"></i>
        <span>Dark Cyber Mode</span>
    </div>

    <!-- Wrapper added for 3D tilt effect -->
    <div class="login-card-wrapper" id="card-wrapper">
        <div class="login-card" id="main-login-card">
            ...
            <form action="<?= htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="POST" class="hologram-form" id="login-form">
                ...
                <!-- Updated for Floating Labels (Input must be BEFORE Label in DOM) -->
            ...
            if (faceIdContainer && faceIdIcon && biometricStatus) {
                faceIdContainer.addEventListener('click', () => {
                    ...
                    setTimeout(() => {
                        ...
                        faceIdIcon.className = 'ph ph-check-circle hologram-face-icon'; // Changes to checkmark
                        biometricStatus.innerText = 'Verification complete. Fill form details below.';
                        biometricStatus.classList.remove('scanning');
                        
                        setTimeout(() => {
                            faceIdIcon.className = 'ph ph-scan-face hologram-face-icon'; // Reset after a moment
